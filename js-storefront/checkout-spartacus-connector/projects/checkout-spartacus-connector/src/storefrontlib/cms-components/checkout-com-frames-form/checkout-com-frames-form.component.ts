...
import {
  ...
  FramesStyle,
  FramesCardholder, FrameCardBindChangedEvent
} from './interfaces';
import { Subject, BehaviorSubject, Observable } from 'rxjs';
import { takeUntil, switchMap, skipWhile, first, filter, map } from 'rxjs/operators';
...
export class CheckoutComFramesFormComponent implements OnInit, AfterViewInit, OnDestroy {
  ...
  private cardBinChanged = new Subject<FrameCardBindChangedEvent>();
  ...
  showTooltip = false;
  isABC: boolean;
  cardHolderTooltipLabel = 'paymentForm.frames.cardHolderLabelTooltip';
  cardHolderTooltipText = 'paymentForm.frames.cardHolderTooltip';
  ...
  private getIsABCParam() {
    this.userIdService.getUserId().pipe(
      ...
      }, err => console.error('getIsABCParam with errors', { err })
    );
  }
  ...
  private setConfig(initialConfig: FramesConfig) {
    ...
    config.cardBinChanged = (e) => this.cardBinChanged.next(e);
  ...
  private listenForFramesEvents() {
    ...
    this.cardBinChanged.pipe(takeUntil(this.drop)).subscribe(event => {
