...
import { getOccIsABC, getOccMerchantKey, getPaymentAddress, getPaymentDetails } from '../store/checkout-com.selectors';
import { Observable, throwError, of, pipe } from 'rxjs';
import { SetOccMerchantKey, SetPaymentAddress, CreatePaymentDetails, CreateApmPaymentDetails, CheckoutUpdatePaymentDetails, SetOccIsABCSuccess, SetOccIsABC } from '../store/checkout-com.actions';
import { ActiveCartService, Address, UserIdService, PaymentDetails, OCC_USER_ID_ANONYMOUS } from '@spartacus/core';
import { switchMap, first, takeUntil, take, map } from 'rxjs/operators';
...
export class CheckoutComPaymentService {
  ...
  public createPaymentDetails(paymentDetails: CheckoutComPaymentDetails, userId: string, cartId: string): Observable<PaymentDetails> {
    this.checkoutComStore.dispatch(new CreatePaymentDetails({
      paymentDetails,
      cartId,
      userId
    }));
  ...
  public createApmPaymentDetails(paymentDetails: ApmPaymentDetails, cartId: string, userId: string): Observable<PaymentDetails> {
    this.checkoutComStore.dispatch(new CreateApmPaymentDetails({
      paymentDetails,
      cartId,
      userId
    }));
  ...
  public updatePaymentDetails(paymentDetails: CheckoutComPaymentDetails): Observable<void> {
    return this.userIdService.getUserId().pipe(
        map(userId =>
          this.checkoutComStore.dispatch(new CheckoutUpdatePaymentDetails({
            userId,
            paymentDetails
          }))
        )
  ...
  public getIsABC(userId: string): void {
    this.checkoutComStore.dispatch(new SetOccIsABC({userId}));
  }

  public getIsABCFromState(): Observable<boolean> {
    return this.checkoutComStore.pipe(select(getOccIsABC));
  }
}
